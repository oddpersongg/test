...
- **Service table struct** (`Bl_Dcm_Service_t` + `Bl_Uds_ServiceFunc_t`, defined in `Config/Bl_Dcm_Lcfg.h`, instantiated in `Bl_Dcm_Lcfg.c`): discrimination metadata only — SID / sub-function byte count (`u8_SubFuncLen`, 0 = no sub-function, 1 = 1-byte sub-function — ISO 14229 sub-functions are always 1 byte) / suppress-positive-response flag (`e_SuppressBit`, `Bl_Dcm_SuppressBitType`: `ENABLE` (e.g. 3E 80) / `DISABLE` (0x80 set → NRC 0x12)) / min length / session mask (`e_SessionMask`, `Bl_Dcm_SessionMaskType` bits: `BL_DCM_SESSION_MASK_DEFAULT/PROGRAMMING/EXTENDED`, ORed in the config, `BL_DCM_SESSION_MASK_ALL` shorthand) / security needed / handler fn. **Supported sub-functions are NOT in this table**: the Dcm 0x12 gate calls `Bl_UdsService_Find(sid, sub)` against the UdsService sub-service table — each sub-function id is defined in exactly one place. **Add/remove/re-configure services by editing `Bl_Dcm_Lcfg.c` (+ the sub-service table in `Bl_UdsService_Lcfg.c`) only — no Core changes needed.** (P2/P2* per-service override fields were removed — no consumer yet; re-add together with the 0x78 pending path in `Bl_Dcm_MainFunction`, driven by the `Bl_TimingManager` P2/P2* timers.)
- **NRC gating order**: SID not found → 0x11 serviceNotSupported; request shorter than min → 0x13 incorrectMessageLength; sub-function violations → 0x12 subFunctionNotSupported (the sub-function is looked up in the service's sub-service id table from `Bl_UdsService_Lcfg`; a 0x80 suppress bit on a service with `e_SuppressBit == DISABLE` is also rejected); session not allowed → 0x7F serviceNotSupportedInActiveSession; security needed and not unlocked → 0x33 securityAccessDenied. Pass → call the `Bl_Uds` handler.
