...
1. `s_Bl_Rte_SysInit()` → `Bl_DriverAdapter_Init()` — OLED init + start TIM1 interrupt + `Bl_Can_Init()` (filter + start + RX interrupt)
...
│   │   ├── Bl_DriverAdapter  Driver init/deinit hub (CDD + Timer + CAN)
...
- `Bl_DriverAdapter_Init()` runs three parallel static init functions (single return via `e_Ret |=`):
...
- **Bl_Uds services implemented**: 0x10 session (0x50+sub+P2+P2*), 0x11 reset (sub 01), 0x27 security (fixed seed/key), 0x34 request download (range check + 0x74 maxBlockLength), 0x36 transfer data (block sequence + length + dest-range checks; flash write TODO), 0x37 transfer exit, 0x3E tester present (suppress-bit). NRC helper `Bl_Uds_SendNrc`.
...
2. UDS services: 0x10/0x11/0x27/0x34/0x36/0x37/0x3E **done in Bl_Uds** (discrimination in Bl_Dcm); remaining: 0x36 actual flash write via Bl_Flash (Apdapter), 0x11 reset trigger, real seed/key algorithm, 0x22/0x2E/0x31.
3. Flash driver (Apdapter): 2KB page erase/write/verify for downloaded data (`Bl_Flash_ErasePage/Write/Verify/JumpToApp`), wired into Bl_Uds 0x36.
